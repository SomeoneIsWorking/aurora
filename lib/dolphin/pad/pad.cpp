...
// Default keyboard layout (previously every slot was PAD_KEY_INVALID, so an
// "active" keyboard pad did nothing until the user hand-authored
// keyboard_bindings.dat):
//   move = WASD    camera (C-stick) = IJKL    d-pad = arrows
//   A = Space   B = LCtrl   X = E   Y = Q   Z = C
//   START = Enter   L = F   R (FLUDD spray) = LShift
std::array<PADKeyButtonBinding, PAD_BUTTON_COUNT> g_defaultKeys{{
    {SDL_SCANCODE_SPACE, PAD_BUTTON_A},
    {SDL_SCANCODE_LCTRL, PAD_BUTTON_B},
    {SDL_SCANCODE_E, PAD_BUTTON_X},
    {SDL_SCANCODE_Q, PAD_BUTTON_Y},
    {SDL_SCANCODE_RETURN, PAD_BUTTON_START},
    {SDL_SCANCODE_C, PAD_TRIGGER_Z},
    {SDL_SCANCODE_F, PAD_TRIGGER_L},
    {SDL_SCANCODE_LSHIFT, PAD_TRIGGER_R},
...
std::array<PADKeyAxisBinding, PAD_AXIS_COUNT> g_defaultKeyAxis{{
    ...
    {SDL_SCANCODE_F, PAD_AXIS_TRIGGER_L, 0},
    {SDL_SCANCODE_LSHIFT, PAD_AXIS_TRIGGER_R, 0},
}};
